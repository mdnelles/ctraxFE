...
export const moneyFormat = (num, sigDigs, doLetters) => {
  ...
  if (doLetters === undefined || doLetters === true) {
    nn.replace(',000,000,000', 'B').replace(',000,000', 'M').replace(',000', 'k');
  }
  if (
    nn[nn.length - 4] === ',' &&
    nn[nn.length - 2] === '0' &&
...
export const formatNumber = (
  ...
  const removeTrailingZeros = (number) => +number.toFixed(isDays ? 0 : toFixed).toString();
  ...
  return withUnits
    ? formatNumberWithUnits(result, { isPercent, isDays, noSign, isNumPositive: num >= 0 })
    : result;
};
